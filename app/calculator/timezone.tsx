  ...
  return (
    <View style={{ flex: 1, backgroundColor: theme.bg }}>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={[styles.title, { color: theme.text }]}>世界時間</Text>
        <Text style={[styles.subtitle, { color: theme.textMuted }]}>看看世界各地現在幾點</Text>
        ...
        <Text style={[styles.sectionLabel, { color: theme.textMuted }]}>其他城市</Text>
        <View style={styles.list}>
          {CITIES.slice(1).map((c) => {
            const offset = getOffset(now, c.tz);
            const sign = offset >= 0 ? '+' : '';
            return (
              <View key={c.tz} style={[styles.row, { backgroundColor: theme.cardBg }]}>
                <View style={styles.rowLeft}>
                  <Clock size={20} color="#2c5fa8" weight="duotone" />
                  <View>
                    <Text style={[styles.rowCity, { color: theme.text }]}>{c.city}</Text>
                    <Text style={[styles.rowOffset, { color: theme.textMuted }]}>台北 {sign}{offset} 小時</Text>
                  </View>
                </View>
                <View style={{ alignItems: 'flex-end' }}>
                  <Text style={[styles.rowTime, { color: theme.text }]}>{formatTime(now, c.tz)}</Text>
                  <Text style={[styles.rowDate, { color: theme.textMuted }]}>{formatDayDate(now, c.tz)}</Text>
                </View>
              </View>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  rowCity: { fontFamily: 'Fredoka_600SemiBold', fontSize: 16 },
  rowOffset: { fontFamily: 'Fredoka_400Regular', fontSize: 11, marginTop: 2 },
  rowTime: { fontFamily: 'Fredoka_700Bold', fontSize: 22, letterSpacing: -0.5 },
  rowDate: { fontFamily: 'Fredoka_400Regular', fontSize: 11, marginTop: 2 },
});
